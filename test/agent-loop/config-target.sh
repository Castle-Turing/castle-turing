...
render_prompt() {
  ...
  case "$1" in
    usable)  env HOME="$RENDER_HOME" CASTLE_REQUEST_ID=probe CASTLE_DIFF_FILE="$RENDER_HOME/diff" CASTLE_TARGET_FILE="$RENDER_HOME/target" CASTLE_FINDING_FILE="$RENDER_HOME/finding" CASTLE_EDIT_DIR="$RENDER_HOME/edit" \
               CASTLE_PRIVATE_ROOT="$PRIVATE" CASTLE_MECHANISM_ROOT="$MECHANISM" \
               bash "$PROMPT_RENDER" <<<"$packet" 2>&1 ;;
    invalid) env HOME="$RENDER_HOME" CASTLE_REQUEST_ID=probe CASTLE_DIFF_FILE="$RENDER_HOME/diff" CASTLE_TARGET_FILE="$RENDER_HOME/target" CASTLE_FINDING_FILE="$RENDER_HOME/finding" CASTLE_EDIT_DIR="$RENDER_HOME/edit" \
    ...
    absent)  env -u CASTLE_MECHANISM_ROOT -u CASTLE_MECHANISM_ROOT_INVALID \
               HOME="$RENDER_HOME" CASTLE_REQUEST_ID=probe CASTLE_DIFF_FILE="$RENDER_HOME/diff" CASTLE_TARGET_FILE="$RENDER_HOME/target" CASTLE_FINDING_FILE="$RENDER_HOME/finding" CASTLE_EDIT_DIR="$RENDER_HOME/edit" \
