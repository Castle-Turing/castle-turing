...
**A drafted design exists.** This was briefly promoted to a numbered
brief (the storage-canary daemon: `mlockall`ed, write+fsync probes
per watched mount, an alarm path down `/dev/kmsg` and `/dev/pts` that
touches no disk, an optional sysrq-panic escalation, and a
dm-linear-backed VM test), cross-reviewed twice, then returned here
to be prioritized as hygiene rather than scheduled. That draft —
worth starting from, not redoing — lives on the unmerged branch
`0074-a-dead-disk-reads-as-a-quiet-night` (PR #131, closed). Recover
it when this entry is specced.
